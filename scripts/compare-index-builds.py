...
def main():
    ...
    for repetition in range(args.repetitions):
        for name in (['before', 'after'] if repetition % 2 == 0 else ['after', 'before']):
            ...
            indexes.mkdir()
            env = {**base_env, 'FXI_INDEXES': str(indexes), 'FXI_QUERY_LOCAL': '1' if (args.candidate_query_local if name == 'after' else args.baseline_query_local) else '0', 'FXI_GENERATION_ROUTING': '1' if (args.candidate_generation_routing if name == 'after' else args.baseline_generation_routing) else '0'}
    ...
    result = {'corpus': str(root), 'corpus_manifest': manifest,
              ...
              'candidate_query_local': args.candidate_query_local, 'baseline_query_local': args.baseline_query_local, 'candidate_generation_routing': args.candidate_generation_routing, 'baseline_generation_routing': args.baseline_generation_routing,
