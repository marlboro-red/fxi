...
try:
    ...
    for pattern, mode in [(p,m) for p in patterns for m in ['regex','phrase']]:
        oracle = subprocess.run(['rg', '-F', '-l', '--color=never', '--', pattern, '.'], cwd=root, capture_output=True, timeout=120)
        ...
        for rep in range(-1, args.repetitions):
            ...
            for name in order:
                ...
                variant = json.dumps(pattern) if mode == 'phrase' else "re:/" + re.escape(pattern).replace("/", "\\/") + "/"
